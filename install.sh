...
ACRE Masternode installer arguments:
...
echo "
 ...
 | the ACRE Team CANNOT assist you in repairing     |::
...
echo "" && echo 'Added user "acre"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./acred /usr/local/bin
mv ./acre-cli /usr/local/bin
mv ./acre-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .acre directory
mkdir $USERHOME/.acre
...
# Create acre.conf
touch $USERHOME/.acre/acre.conf
cat > $USERHOME/.acre/acre.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
EOL
chmod 0600 $USERHOME/.acre/acre.conf
chown -R $USER:$USER $USERHOME/.acre

sleep 1

cat > /etc/systemd/system/acre.service << EOL
[Unit]
Description=acred
...
ExecStart=/usr/local/bin/acred -conf=${USERHOME}/.acre/acre.conf -datadir=${USERHOME}/.acre
ExecStop=/usr/local/bin/acre-cli -conf=${USERHOME}/.acre/acre.conf -datadir=${USERHOME}/.acre stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable acre.service
sudo systemctl start acre.service
